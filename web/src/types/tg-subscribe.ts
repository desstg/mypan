...
export interface TGConfig {
  ...
  last_poll_at: string;
  /**
   * 「热门推荐」的海报墙能不能用，取决于两件**别处**的配置：
   * TMDB 的 API Key（在「目录整理 → 整理设置」里填）与全局代理
   * （在「系统设置 → 网络代理」里开）。都不属于 TG 订阅自己的配置，
   * 但这一页要用它们，所以后端在同一个响应里报出来。
   *
   * 报的是**设置填没填**，不是**能不能连上** —— 后者要真发请求。
   */
